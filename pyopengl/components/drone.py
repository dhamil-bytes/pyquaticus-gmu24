...
class Drone(GameObject):
    ...
    def check_divider_wall_collision(self, new_x=None, new_y=None, new_z=None):
        ...
        # list of booleans to ID collisions
        collisions = [False, False, False]
        
        # Check if drone would cross the wall
        if (new_x):
            i = 0
            wall_x = wall.position[i]
            current_x = self.position[i]
    ...
    def update_captured_flag_position(self):
        if self.captured_flag:
            ...
            self.captured_flag.position[1] = self.position[1] + self.speed * offset # Same height as drone
    ...
    def move_forward(self):
        ...
        if self.environment:
            ...
            if (-half_width < new_x < half_width and 
                ...
                collision_x, _, _ = self.check_divider_wall_collision(new_x)
                
                if not collision_x and \
    ...
    def move_upward(self):
        ...
        if self.environment:
            ...
            if (-half_height < new_y < half_height and 
                ...
                # Check for divider wall and drone collisions first
                _, collision_y, _ = self.check_divider_wall_collision(new_y)

                if not collision_y and \
                   not self.check_drone_collision(self.position[0], new_y, self.position[2]):
